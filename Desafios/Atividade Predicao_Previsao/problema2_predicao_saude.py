...
Problema 2: Predição de Risco de Internação (Risco_Internacao)

Fonte de dados:
https://github.com/alexandrezamberlan/tias/blob/main/3_predicao_previsao_codigos_exemplos/dados_saude_predicao.csv

X (features): Idade, Pressao_Arterial, Colesterol_Total, Frequencia_Cardiaca_Max
y (target):   Risco_Internacao (0 = baixo risco, 1 = alto risco)
"""
...
print(df["Risco_Internacao"].value_counts(normalize=True))

X = df[["Idade", "Pressao_Arterial", "Colesterol_Total", "Frequencia_Cardiaca_Max"]]
y = df["Risco_Internacao"]
...
for i, (nome, modelo) in enumerate(modelos.items()):
    ...
    print(f"\n===== {nome} =====")
    print(classification_report(y_test, y_pred_test, target_names=["Baixo_Risco", "Alto_Risco"]))

    cm = confusion_matrix(y_test, y_pred_test)
    sns.heatmap(cm, annot=True, fmt="d", cmap="Reds", ax=axes[i],
                xticklabels=["Pred: Baixo", "Pred: Alto"],
                yticklabels=["Real: Baixo", "Real: Alto"])
    axes[i].set_title(f"{nome}\nAcc treino={acc_train:.2f} | Acc teste={acc_test:.2f}")

plt.tight_layout()
plt.savefig("matrizes_confusao_problema2.png", dpi=150)
print("\nFigura salva: matrizes_confusao_problema2.png")

# ---------------------------------------------------------------
# 4. Tabela comparativa final
# ---------------------------------------------------------------
df_resultados = pd.DataFrame(resultados).sort_values("F1_Teste", ascending=False)
print("\n\n========== TABELA COMPARATIVA - PROBLEMA 2 ==========")
print(df_resultados.to_string(index=False))
df_resultados.to_csv("resultados_problema2.csv", index=False)
